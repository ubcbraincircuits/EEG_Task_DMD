...
for r_type = {'rOpt'} % choose 'rOpt'or 'r50'
    for data_type = {'noNorm'} % choose 'noNorm' or 'zscore'
        ...
        Params.ds_factor = 2; % downsampling factor
        Params.TDE_vals = 1;
        ...
        for ds = 1:length(data_sets)
            ...
            for sub = 1:nSub
                ...
                for f = 1:length(FreqStr)
                    ...
                    save_path = sprintf('Results_DMD_rest_%s_%s_DS2/%s/%s/Sub%d/',r_type{1},data_type{1},data_sets{ds},FreqStr{f},sub);
                    mkdir(save_path);
